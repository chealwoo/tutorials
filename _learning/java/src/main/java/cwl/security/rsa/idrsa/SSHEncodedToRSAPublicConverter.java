...
public class SSHEncodedToRSAPublicConverter {
    ...
    public RSAPublicKeySpec convertToRSAPublicKey() {
        try {
            ...
            Preconditions.checkArgument(Iterables.size(parts) >= 2 && SSH_MARKER.equals(Iterables.get(parts,0)), "bad format, should be: ssh-rsa AAAB3....");
            stream = new ByteArrayInputStream(BaseEncoding.base64().decode(Iterables.get(parts, 1)));
            String marker = new String(readLengthFirst(stream));
            Preconditions.checkArgument(SSH_MARKER.equals(marker), "looking for marker %s but received %s", SSH_MARKER, marker);
